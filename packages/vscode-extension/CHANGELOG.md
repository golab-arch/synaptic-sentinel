...
**Empirical classification (Baseline-8d in SENTINEL workspace)**: **H1.5-implicit**. The graph builds correctly with rich data (e.g. `colony-db.ts role=library imports=1 importedBy=3 symbols=14`), the LLM (Sonnet-4.6 via deepseek-v4-flash) produces sophisticated rationales, but does NOT explicitly cite the graph section keywords (`imported by`, `exports`, `library`, `entry`). Interpretation: for intra-file findings (like SENTINEL's `eval-usage` and `taint-command-injection`), the graph context is structurally redundant — the verdict depends on local API usage. R18 v1's real value would surface on cross-module findings; symbol-level cross-file resolution (R18 v2) is documented backlog.

### Added

- **DG-123 A — R18 v1 Interaction Graph Layer** (Cycle 111). New module `@synaptic-sentinel/core` `packages/core/src/coordinator/interaction-graph.ts` (~560 lines): `SUPPORTED_LANGUAGES` = `['typescript', 'tsx', 'javascript', 'python']` + `FileContextSchema` (`language` + `inferredRole` (`entry`/`library`/`test`/`leaf`) + `imports` + `importedBy`) + `SymbolContextSchema` (`definedSymbols` + `exportedSymbols`) + `buildInteractionGraph(rootPath)` two-pass walker (WASM parsing via `web-tree-sitter@0.20.8` + `tree-sitter-wasms@0.1.13` for the 4 grammars, +5.87 MB uncompressed / +750 KB gzipped vsix bloat accepted for real AST fidelity). Skip segments inherited from DG-117 A. Fallback graceful — if WASM fails to load, the graph is empty and findings keep their pre-DG-123 A behavior.

  New Coordinator Stage 1.5b (`runScan`): between the exclude-list filter and Stage 2 dedup, `buildInteractionGraph` is called once per scan; each Finding whose `location.path` matches a graph node gains `fileContext` + `symbolContext` (opt-in, additive). Findings in unsupported languages (Rust, Go, `package-lock.json`, `.yml`, `Dockerfile`) or with parse errors keep both fields `undefined`. The Triage Agent and Context Agent user prompt gain a new "**File interaction context (DG-123 A — system-as-graph)**" section formatted by `formatInteractionContext(finding)`, with caps `MAX_LISTED_IMPORTS_IN_PROMPT = 8`, `MAX_LISTED_IMPORTERS_IN_PROMPT = 8`, `MAX_LISTED_SYMBOLS_IN_PROMPT = 12` (defensive against prompt bloat in hub files).

- **DG-123.0.1 — Interaction Graph telemetry via ScanOptions callbacks** (Cycle 111 sub-hotfix). Coherent with the existing `onScoutSettled` pattern: two new optional `ScanOptions` fields `onInteractionGraphBuilt?: (stats: InteractionGraphStats) => void` and `onFindingEnriched?: (event: FindingEnrichedEvent) => void` — best-effort (try/catch defensive), so callback throws never break the scan. The CLI wires both to `spinner.log('[DG-123 A] ...')` lines that surface in the extension's terminal output. Enables observability of the graph state per scan: `[DG-123 A] graph built: N node(s), M/K finding(s) enrichable, Xms` + per-finding `[DG-123 A] enriched <path> role=<role> imports=N importedBy=M symbols=K`. Without this, silent structural failures of the graph (WASM load errors, path resolution bugs) would be invisible.

- **DG-123.0.2 — Critical resolver fix for TypeScript ESM imports** (Cycle 111 sub-hotfix). Baseline-8c in SENTINEL identified that `resolveImportPath` pre-fix treated file extensions as additive suffixes (e.g. `foo.js` + `.ts` = `foo.js.ts` — inexistent) instead of substitutes for the TypeScript ESM idiom `import { X } from './foo.js'` (where the source file on disk is `foo.ts`, the `.js` only exists post-build in `dist/`). Effect on SENTINEL Baseline-8c: **100% of relative imports failed to resolve** → all 4 enrichable findings showed `imports=0 importedBy=0 role=entry` (cascade of `inferRole` returning `'entry'` when `importedByCount === 0`). New `EXTENSION_SUBSTITUTES` table maps: `.js → [.ts, .tsx, .jsx]`, `.mjs → [.mts]`, `.cjs → [.cts]`, `.jsx → [.tsx]`. `resolveImportPath` refactored into 4 cases with early return. Semantics preserved for all cases that already worked. Baseline-8d confirmed empirically: `colony-db.ts` and `cli-runner.ts` now show `imports=1 importedBy=3 role=library` in the telemetry.

### Changed

- `FindingSchema` extended with optional `fileContext?: FileContext` + `symbolContext?: SymbolContext`. Additive + backward-compatible: findings persisted in `colony.db` before DG-123 A remain valid with both fields `undefined`.
- `ScanOptions` extended with optional `onInteractionGraphBuilt?` + `onFindingEnriched?` callbacks (DG-123.0.1). Additive — callers not passing them see unchanged behavior.
- `packages/vscode-extension/scripts/copy-cli-assets.mjs` now packages 5 additional WASM files (`tree-sitter.wasm` runtime + 4 grammars from `tree-sitter-wasms@0.1.13`, Unlicense/public domain). `web-tree-sitter` added to `externalDeps` and to `--external:` in the esbuild bundle commands. Fail-loud on missing WASM.
- New exports from `@synaptic-sentinel/core`: `SUPPORTED_LANGUAGES`, `FileContextSchema`, `SymbolContextSchema`, `FileSymbolSchema`, `buildInteractionGraph`, `detectLanguage`, `getInteractionContextForPath`, `InteractionGraphStats`, `FindingEnrichedEvent`.

### Known Issues / Honest tradeoffs (anti-optimism active)

- **LLM utilization is workload-dependent** — Baseline-8d in SENTINEL classified as **H1.5-implicit**: with rich graph data (`imports=1 importedBy=3 role=library` for `colony-db.ts` + `cli-runner.ts`), the LLM produces sophisticated FP rationales (`spawn env option, no shell` / `conditionally access __filename`) but does NOT explicitly cite the graph section keywords. The 4 SENTINEL findings are intra-file (verdict depends on local API usage, not module relationships), so the graph context is structurally redundant. R18 v1's real value would surface on **cross-module findings** (e.g. SYNAPTIC_SAAS `agent.ts` calling `agentLoop.execute()` defined in another file) — but v1 only does file-level, not symbol-level cross-file. Symbol-cross-file is documented backlog (R18 v2 in the competitive research doc §12.5).
- **Only 4 languages in v1** — TS/TSX/JS/Python. Findings in Rust, Go, Java, Ruby, C#, Swift, etc. get `fileContext` + `symbolContext` = `undefined` (graceful degradation). Adding grammars is straightforward but each adds 2-3 MB to the vsix; deferred until user demand emerges.
- **Only relative static imports resolve** — bare imports (`react`, `zod`, `express`, `@synaptic-saas/core`) explicitly return `null`. Cross-package internal imports in a pnpm workspace count as bare unless the specifier uses relative paths. Observable effect: `imports=1` for files with 8+ actual imports because 7 are bare. Bare imports as informational metadata is a future refinement (R18 v1.1 candidate).
- **`role='leaf'` semantics ambiguity** — `inferRole` returns `'leaf'` when `importedByCount === 1` (after the `>= 2 → library` check). Conventionally "leaf" implies zero importers. Rough heuristic that works for the intended goal (rough role labels for the LLM) but a purist might refine in v1.1.
- **First-scan latency +500ms-2s** — WASM init cost + graph build cost. SENTINEL (~1200 files) takes ~500-1400ms; larger monorepos can take 5-30s. Not cached across scans (each `runScan` rebuilds). Cache-by-content-hash is a future optimization if user reports pain.
- **Provider intermittent empty responses** (non-DG-123 issue, side observation): during Baseline-8c and 8d, the LLM provider `deepseek-v4-flash` occasionally returned `Respuesta OpenAI-compatible sin texto en choices[0].message.content` for specific findings (`Security check suppressed` persistently 2/2, `CORS` flipped success → failure across runs). Not a graph bug — reproducible with the same provider outside of R18 v1. Separate follow-up if it persists (DG-125 A candidate).
- **`role=library` cascade fixed by DG-123.0.2** — files that pre-fix showed `role=entry` (because `importedByCount === 0` was cascade from the resolver bug) now correctly report `role=library` (importedByCount ≥ 2). Users of step8.vsix / step9.vsix who saw all `role=entry` in the telemetry Output should re-install this release to see accurate roles.

### Notes

- **Empirical validation trajectory**: Baseline-8 in SENTINEL (colony memory pre-cached FP for 4 findings → LLM not invoked) → Baseline-8b in SYNAPTIC_SAAS (single enrichable finding with sparse data attributed at the time to Fastify autoload — later revealed to be part of the DG-123.0.2 resolver bug) → Baseline-8c in SENTINEL post-clear-learning_records (H2-late confirmed with root cause identified: `imports=0 importedBy=0` for all TS findings) → Baseline-8d in SENTINEL post-DG-123.0.2 fix (rich data confirmed: `imports=1 importedBy=3 role=library` for `colony-db.ts` and `cli-runner.ts`; H1.5-implicit classification).
- `pnpm verify` VERDE: 63 test files / **779 tests** + manifest gate + activation gate (9 commands + 15 subscriptions). Test growth from v0.3.16: **+16 tests** (763 → 779), of which **+13** are the interaction-graph fixture matrix (DG-123 A) + **+3** the resolver fix (DG-123.0.2 `.js → .ts / .tsx / .jsx`, `.mjs → .mts`, negative regression).
- `vsce publish` to the VS Code Marketplace is **not** part of this release. The Marketplace listing is on v0.3.3; **15 GitHub-only releases** now accumulate (v0.3.4 → v0.3.17) — new maximum skip range of the project. To install in VS Code, download `synaptic-sentinel-0.3.17.vsix` and use `code --install-extension synaptic-sentinel-0.3.17.vsix` or _Install from VSIX..._ in the Extensions view.
- **Roadmap next**: R18 v1.1 (bare imports as informational metadata + symbol count refinement for entry/library edge cases); R18 v2 (symbol-level cross-file — resolve `agentLoop.execute()` to the defining file's context); DG-125 A (provider intermittent-empty follow-up if it persists).
